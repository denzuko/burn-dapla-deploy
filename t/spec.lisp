...
;;;;
;;;; Full coverage spec for burn-dapla-deploy/deploy exports.
;;;; Run: (asdf:test-system :burn-dapla-deploy/spec)
...
(def-suite :burn-spec
  :description "Full coverage spec for burn-dapla-deploy.")
...
(test network-bridge
  ...
  (let ((ini (cinix-write-string (burn-network-sections))))
    (is (ini-has ini "Driver=bridge"))
    (is (not (ini-has ini "Internal=true")))))

(test network-vlsm
  "Network unit has VLSM subnet 10.89.2.28/30 and gateway 10.89.2.29."
  (let ((ini (cinix-write-string (burn-network-sections))))
...
(test container-home-volume-ro
  ...
  (let* ((ini (cinix-write-string (burn-container-sections)))
         (lines (ini-lines ini))
...
(test container-data-volume-srv
  ...
  (is (ini-has (cinix-write-string (burn-container-sections)) "Volume=/srv/%U")))

(test container-no-publish-port
  "Container unit has no PublishPort — netavark handles routing."
  (is (not (ini-has (cinix-write-string (burn-container-sections)) "PublishPort"))))
...
(test container-image
  "Container unit uses correct OCI image oci.dapla.net/corentinth/enclosed:latest-rootless."
  (is (ini-has (cinix-write-string (burn-container-sections)) "oci.dapla.net/corentinth/enclosed:latest-rootless")))
...
(test haproxy-netavark-backend
  "HAProxy backend targets netavark gateway 10.89.2.29:8080."
  (let ((cfg (haproxy-vhost-config)))
