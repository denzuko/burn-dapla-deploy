Feature: burn-dapla-deploy rootless Podman quadlet deploy
  ...
  Scenario: Network unit uses netavark bridge
    When burn-network-sections is called
  ...
  Scenario: Network unit has correct VLSM allocation
    When burn-network-sections is called
    Then the INI contains "Subnet=10.89.2.28/30"
    And the INI contains "Gateway=10.89.2.29"

  Scenario: Container home profile is read-only via %%h
    When burn-container-sections is called
  ...
  Scenario: HAProxy backend targets netavark gateway
    ...
    Then the output contains "10.89.2.29:8080"
